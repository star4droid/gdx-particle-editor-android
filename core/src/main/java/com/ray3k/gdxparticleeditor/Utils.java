...
/**
 * A convenience class with various static methods that perform various utility
 * tasks throughout GDX Particle Editor.
 */
public class Utils {

    public static final int[] EMPTY_KEYBIND = new int[]{};

    public static void setButtonSize(Button btn, float width, float height) {
        btn.setSize(width, height);
        if (btn.getStyle().up != null) {
            btn.getStyle().up.setMinWidth(width);
            btn.getStyle().up.setMinHeight(height);
        }

        if (btn.getStyle().down != null) {
            btn.getStyle().down.setMinWidth(width);
            btn.getStyle().down.setMinHeight(height);
        }

        if (btn.getStyle().over != null) {
            btn.getStyle().over.setMinWidth(width);
            btn.getStyle().over.setMinHeight(height);
        }

        if (btn.getStyle().focused != null) {
            btn.getStyle().focused.setMinWidth(width);
            btn.getStyle().focused.setMinHeight(height);
        }
    }
    ...
    public static void updateViewportScale(float scale) {
        updateViewportScale(valueToUIscale(scale));
    }
    ...
    public static void checkVersion(VersionUpdateRunnable updater) {
        if (!preferences.getBoolean(
                Settings.NAME_CHECK_FOR_UPDATES, Settings.DEFAULT_CHECK_FOR_UPDATES)) {
            return;
        }
                ...
                = new Thread(
                        () -> {
                            ...
                            = requestBuilder
                                    ...
                                    .url(
                                            "https://raw.githubusercontent.com/libgdx/gdx-particle-editor/master/core/src/main/resources/version")
                                    .build();
    ...
    public static void initShaderProgram() {
        var vertex
                = vertShaderFile == null
                        ? spriteBatch.getShader().getVertexShaderSource()
                        : vertShaderFile.readString();
        var frag
                = fragShaderFile == null
                        ? spriteBatch.getShader().getFragmentShaderSource()
                        : fragShaderFile.readString();
    ...
    public static boolean loadParticle(
            FileHandle fileHandle,
            ObjectMap<String, FileHandle> imageFileMap,
            boolean showLocateImagesPop) {
        ...
        try {
            if (fileHandle.type() != FileType.Internal) {
                ...
                if (imageFileMap == null) {
                    newParticleEffect.loadEmitterImages(fileHandle.parent());
                }else {
                    ...
                    for (int i = 0, n = newParticleEffect.getEmitters().size; i < n; i++) {
                        ...
                        for (String imagePath : emitter.getImagePaths()) {
                            ...
                            if (sprite == null) {
                                throw new IllegalArgumentException("Image not found: ".concat(imageName));
                            }
            ...
            if (showLocateImagesPop) {
                var pop
                        = new PopImageError(
                                "Error loading particle file. Ensure that all associated images are saved locally.",
                                e.getMessage(),
                                fileHandle,
                                false);
    ...
    public static boolean mergeParticle(
            FileHandle fileHandle,
            ObjectMap<String, FileHandle> imageFileMap,
            boolean showLocateImagesPop) {
        var newParticleEffect = new ParticleEffect();
        ...
        try {
            ...
            if (showLocateImagesPop) {
                var pop
                        = new PopImageError(
                                "Error merging particle file. Ensure that all associated images are saved locally.",
                                e.getMessage(),
                                fileHandle,
                                true);
                pop.show(foregroundStage);
        ...
        for (var emitter : particleEffect.getEmitters()) {
            emitter.setPosition(
                    oldActiveEmitters.orderedKeys().first().getX(),
                    oldActiveEmitters.orderedKeys().first().getY());
            activeEmitters.put(emitter, true);
        ...
        for (var oldActiveEmitter : oldActiveEmitters) {
            ...
            if (oldActiveEmitter.value) {
                particleEffect.getEmitters().add(oldActiveEmitter.key);
            }
        }
    ...
    public static void reloadSprites() {
        for (var emitter : particleEffect.getEmitters()) {
            ...
            for (String imagePath : emitter.getImagePaths()) {
                String imageName = new File(imagePath.replace('\\', '/')).getName();
                FileHandle imageHandle = fileHandles.get(imageName);
                if (imageHandle != null) {
                    Sprite sprite = new Sprite(new Texture(imageHandle));
    ...
    private static String getShortcutPreferenceString(String name, boolean primary) {
        return primary ? name.concat("Shortcut") : name.concat("Shortcut").concat("Secondary");
    }
    ...
    public static void changeKeybind(
            KeyMap keyMap, Shortcut s, int[] unsortedKeybind, boolean primary, boolean flush) {
        keyMap.changeKeybind(s, unsortedKeybind, primary);
        writeKeybindToPreferences(s.getName(), unsortedKeybind, primary, flush);
    }

    public static void changeKeybind(
            KeyMap keyMap, Shortcut s, int packedKeybind, boolean primary, boolean flush) {
    ...
    public static void writeKeybindToPreferences(
            String name, int[] keybind, boolean primary, boolean flush) {
        writeKeybindToPreferences(name, ShortcutManager.packKeybindUnsorted(keybind), primary, flush);
    }

    public static void writeKeybindToPreferences(
            String name, int packed, boolean primary, boolean flush) {
    ...
    public static Shortcut createShortcut(
            String name,
            String toolTipDesc,
            int[] defaultPrimaryKeybind,
            int[] defaultSecondaryKeybind,
            int scope,
            Runnable runnable) {
        ...
        if (packed == 0 && defaultPrimaryKeybind != null) {
            ...
            s.setPrimaryKeybind(
                    defaultPrimaryKeybind, ShortcutManager.packKeybindSorted(defaultPrimaryKeybind));
        } else {
        ...
        if (packed == 0 && defaultSecondaryKeybind != null) {
            ...
            s.setSecondaryKeybind(
                    defaultSecondaryKeybind, ShortcutManager.packKeybindSorted(defaultSecondaryKeybind));
        } else {
            int[] unpacked = ShortcutManager.unpacKeybind(packed);
