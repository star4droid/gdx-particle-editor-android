...
public class SaveRunnable implements Runnable {
    ...
    public void setOnCompletionRunnable(Runnable run){
    ...
    public void run () {
        if (openFileFileHandle != null) {
            if (preferences.getBoolean(NAME_PRESUME_FILE_EXTENSION, DEFAULT_PRESUME_FILE_EXTENSION) && !openFileFileHandle.extension().equals("p"))
                openFileFileHandle = openFileFileHandle.sibling(openFileFileHandle.name().concat(".p"));
            ...
            showToast("Saved ".concat( openFileFileHandle.name()));
