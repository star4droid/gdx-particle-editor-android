...
/**
 * PopTable used to change the app settings. These include options for the UndoManager, default
 * screen, checking for updates, UI scale, keyboard shortcuts, and more. Links to related
 * directories in the user folder are provided for convenience.
 */
public class PopEditorSettings extends PopTable {
  ...
  public PopEditorSettings(boolean bool) {
    this();
    resetScale = bool;
  }
  ...
  private void populate() {
    ...
    addListener(
        new TableShowHideListener() {
          ...
          public void tableShown(Event event) {
            if (resetScale && preferences.getFloat(NAME_SCALE, DEFAULT_SCALE) != 1)
              Utils.updateViewportScale(1);
          }
    ...
    ScrollPane scrollPane = new ScrollPane(settingsTable, skin, "emitter-properties");
    ...
    addTooltip(
        spinner,
        "The maximum number of undos that will be kept in memory",
        Align.top,
        Align.top,
    ...
    addTooltip(
        selectBox,
        "The default screen that the app opens to",
        Align.top,
        Align.top,
        tooltipBottomArrowStyle);
    onChange(
    ...
    checkForUpdatesCheckBox.setChecked(
        preferences.getBoolean(NAME_CHECK_FOR_UPDATES, DEFAULT_CHECK_FOR_UPDATES));
    checkBoxTable.add(checkForUpdatesCheckBox);
    addHandListener(checkForUpdatesCheckBox);
    addTooltip(
        checkForUpdatesCheckBox,
        "Whether or not the app checks to see if there is an update available at startup",
        Align.top,
        Align.top,
        tooltipBottomArrowStyle);
    onChange(
    ...
    addTooltip(
        presumeFileExtensionCheckBox,
        "Whether or not the app defaults all particle file dialogs to use the \".p\" file"
            + " extension",
        Align.top,
        Align.top,
        tooltipBottomArrowStyle);
    onChange(
        ...
        () -> {
          preferences.putBoolean(
              NAME_PRESUME_FILE_EXTENSION, presumeFileExtensionCheckBox.isChecked());
          preferences.flush();
        });
    ...
    addTooltip(
        ...
        "Whether or not the app copies the particle images to the directory of the saved particle"
            + " file",
        Align.top,
        Align.top,
        tooltipBottomArrowStyle);
    ...
    addTooltip(
        slider,
        "Increase the UI Scale for high DPI displays",
        Align.top,
        Align.top,
        tooltipBottomArrowStyle);
    ...
    addTooltip(
        textButton,
        "Apply the UI Scale for high DPI displays",
        Align.top,
        Align.top,
        tooltipBottomArrowStyle);
    onChange(
    ...
    shortcutTable.columnDefaults(3).width(90);
    shortcutTable.columnDefaults(4).width(90);
    shortcutTable.columnDefaults(5).uniformX();
    shortcutTable.defaults().space(5);
    add(shortcutTable).padTop(20);

    label = new Label("SHORTCUTS", skin, "header");
    shortcutTable.add(label).colspan(8).align(Align.center);
    ...
    shortcutTable.add();
    boolean addRow = true;
    // 1 row contains 2 shortcut options...
    for (Shortcut s : keyMap.getAllShortcuts()) {
      createShortcutSettingsOption(s, shortcutTable, addRow);
      addRow = (!addRow);
    }
    ...
    addTooltip(
        subButton,
        "Open the preferences directory where GDX Particle Editor saves its settings",
        Align.top,
        Align.top,
        tooltipBottomArrowStyle);
    onChange(
    ...
    addTooltip(
        subButton,
        "Open the log directory where GDX Particle Editor saves errors",
        Align.top,
        Align.top,
        tooltipBottomArrowStyle);
    onChange(
    ...
    addTooltip(
        subButton,
        "Open the GitHub page for GDX Particle Editor",
        Align.top,
        Align.top,
        tooltipBottomArrowStyle);
    onChange(
    ...
    addHandListener(subButton);
    addTooltip(
        subButton, "Close the settings dialog.", Align.top, Align.top, tooltipBottomArrowStyle);
            ...
            Actions.repeat(
                ...
                Actions.sequence(
                    Actions.run(
                        () ->
                            label.setText(
                                "Click to confirm scale.\nResetting in "
                                    .concat(String.valueOf(--confirmTime + 1))
                                    .concat("..."))),
                    Actions.delay(1f))),
            Actions.run(
                () -> {
  ...
  private void createShortcutSettingsOption(
      Shortcut shortcut, Table shortcutTable, boolean addRow) {
    if (addRow) shortcutTable.row();

    var label = new Label(shortcut.getName().concat(":"), skin);
    ...
    addTooltip(
        primaryTextField, shortcut.getDescription(), Align.top, Align.top, tooltipBottomArrowStyle);
    onClick(
        primaryTextField, () -> showKeyBindPop(primaryTextField, shortcut, foregroundStage, true));
    ...
    onClick(
        secondaryTextField,
        () -> showKeyBindPop(secondaryTextField, shortcut, foregroundStage, false));
  }
  ...
  public static void openFileExplorer(FileHandle startDirectory) throws IOException {
    if (startDirectory.exists()) {
      ...
      throw new IOException("Directory doesn't exist: ".concat(startDirectory.path()));
    }
  }
  ...
  private void setShortcutTextFieldText(
      TextField textField,
      String modifiersKey,
      String shortcutKey,
    ...
    if (preferences.contains(modifiersKey) && preferences.contains(shortcutKey)) {
      textField.setText(
          constructShortcutText(
              readModifierText(preferences.getString(modifiersKey)),
  ...
  private void showKeyBindPop(
      TextField textField, Shortcut shortcut, Stage stage, boolean primary) {
        ...
        new InputListener() {
          ...
          public boolean keyDown(InputEvent event, int keycode) {
            ...
            if (Gdx.input.isKeyPressed(Keys.CONTROL_LEFT)
                || Gdx.input.isKeyPressed(Keys.CONTROL_RIGHT)) intArray.add(Keys.CONTROL_LEFT);
