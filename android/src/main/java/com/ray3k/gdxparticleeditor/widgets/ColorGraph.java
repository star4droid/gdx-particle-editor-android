...
/**
 * A widget that allows the user to modify colors over a timeline. Nodes can be added and removed
 * from the timeline, with each being able to be individually repositioned. Clicking a node opens a
 * color pop that allows the user to select a new color.
 */
public class ColorGraph extends Table {
  ...
  public void initialize() {
    ...
    public Array<ImageButton> getNodes(){
        return nodes;
    }
  ...
  private NodeData createNode(float value, Color color, boolean stationary) {
    ...
    if (nodeData.color == null) {
      var previousColor =
          previousNode != null ? ((NodeData) previousNode.getUserObject()).color : Color.RED;
                                    ...
                                    cp.addListener(
                                        new PopColorPickerListener() {
                                          ...
                                          public void cancelled(Color oldColor) {
                                            ...
                                            fire(new ChangeEvent());
                                            fire(
                                                new ColorGraphEvent(CHANGE_CANCEL, nodeData.color));
                                            openColorPicker = true;
                                            createNewNode = false;
                                          }
                                        });
    ...
    clickListener.setTapSquareSize(60);
    ...
    dragListener.setTapSquareSize(60);
  ...
  public static class ColorGraphEvent extends Event {
    public ColorGraphEvent(ColorGraphEventType type,Color color){
        this.color = color;
        this.type = type;
    }
  ...
  public abstract static class ColorGraphListener implements EventListener {
    @Override
