...
/**
 * The introductory table which highlights the app title and buttons to open the classic or wizard
 * modes.
 */
public class WelcomeTable extends Table {
  ...
  public WelcomeTable() {
    if (preferences.getFloat(NAME_SCALE, DEFAULT_SCALE) != 1) Utils.updateViewportScale(1);
    ...
    var image = new Button(skin, "title");
    table.add(image);
    ...
    var classicCard =
        new WelcomeCard(
            "Classic",
            "The classic Particle Editor experience",
            skin.getDrawable("thumb-classic"),
            "Open Classic Mode");
    ...
    var wizardCard =
        new WelcomeCard(
            "Wizard",
            "Simple with a large preview",
            skin.getDrawable("thumb-wizard"),
            "Open Wizard Mode");
    ...
    checkbox.setChecked(
        !preferences
            .getString(NAME_OPEN_TO_SCREEN, DEFAULT_OPEN_TO_SCREEN)
            .equals(DEFAULT_OPEN_TO_SCREEN));
    ...
    root.add(settingsButton).expand().bottom().right().padRight(50).padBottom(50);
    Utils.setButtonSize(settingsButton, 50, 50);
    ...
    onChange(
        ...
        () -> {
          ...
          var pop = new PopEditorSettings(false);
