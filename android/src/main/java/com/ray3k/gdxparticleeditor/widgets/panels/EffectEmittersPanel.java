...
/**
 * A widget that displays the emitters attached to the loaded particle effect. User may make new
 * emitters or duplicate existing ones. They can drag the emitters to reorder them or use the
 * buttons to move them up and down. Each emitter can be deleted by selecting and clicking delete or
 * dragging it off the panel. Options to save the particle effect, open a particle file, or merge a
 * file with the open one are available as well.
 */
public class EffectEmittersPanel extends Panel {
  ...
  public EffectEmittersPanel() {
    ...
    emittersDraggableList.addListener(
        new DraggableListListener() {
          @Override
          public void removed(Actor actor, int index) {
            if (particleEffect.getEmitters().size <= 1) return;

            UndoManager.add(
                new DeleteEmitterUndoable(
                    activeEmitters.orderedKeys().get(index), index, "Delete Emitter"));
          ...
          public void reordered(Actor actor, int indexBefore, int indexAfter) {
            ...
            UndoManager.add(
                new MoveEmitterUndoable(emitter, indexBefore, indexAfter, "Move Emitter"));
  ...
  private void showPopEmitterControls(Actor attachToActor) {
    ...
    popEmitterControls.addListener(
  ...
  private void addEmitterButtons(Table table) {
    ...
    onChange(
        ...
        () -> {
          UndoManager.add(
              new NewEmitterUndoable(new ParticleEmitter(selectedEmitter), "Duplicate Emitter"));
    ...
    onChange(
        ...
        () -> {
          UndoManager.add(
              new DeleteEmitterUndoable(
                  selectedEmitter,
                  activeEmitters.orderedKeys().indexOf(selectedEmitter, true),
                  "Delete Emitter"));
    ...
    table.row();
    Image image = new Image(skin, "divider-10");
    ...
    onChange(
        ...
        () -> {
          ...
          pop.show(foregroundStage);
          pop.addListener(
              new PopTable.TableShowHideListener() {
                @Override
                public void tableShown(Event event) {}
    ...
    onChange(
        ...
        () -> {
          ...
          UndoManager.add(
              new MoveEmitterUndoable(selectedEmitter, oldIndex, oldIndex - 1, "Move Up Emitter"));
          populateEmitters();
    ...
    onChange(
        ...
        () -> {
          ...
          UndoManager.add(
              new MoveEmitterUndoable(
                  selectedEmitter, oldIndex, oldIndex + 1, "Move Down Emitter"));
          populateEmitters();
  ...
  public void populateEmitters() {
    ...
    for (int i = 0; i < activeEmitters.orderedKeys().size; i++) {
      ...
      var editableLabel =
          new EditableLabel(emitter.getName(), editableLabelStyle) {
            @Override
            public void unfocused() {
              ...
              if (getText().equals("")) {
                setText("Untitled");
                UndoManager.add(
                    new RenameEmitterUndoable(
                        emitter, emitter.getName(), getText(), "Rename Emitter"));
      ...
      editableLabel.textField.addListener(
          new InputListener() {
            @Override
          ...
          () -> {
            addDelayedUndoAction(
                () -> {
                  UndoManager.add(
                      new RenameEmitterUndoable(
                          emitter, emitter.getName(), editableLabel.getText(), "Rename Emitter"));
                  dragLabel.setText(emitter.getName());
                  dragLabel.pack();
                  removeLabel.setText(emitter.getName());
                  removeLabel.pack();
      ...
      onClick(
          ...
          () -> {
            ...
            emittersDraggableList.getDragAndDrop().setTapSquareSize(TAP_HEIGHT_RENAMING);
  ...
  private void addDelayedUndoAction(Runnable runnable) {
    if (delayedUndoAction != null) delayedUndoAction.restart();
      ...
      delayedUndoAction =
          Actions.sequence(
              Actions.delay(DELAYED_UNDO_DELAY),
              Actions.run(runnable),
              Actions.run(() -> delayedUndoAction = null));
