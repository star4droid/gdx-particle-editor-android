...
/**
 * A widget to display the properties of the selected emitter. A set of default properties are
 * always available while additional properties must be added manually. Each property is represented
 * by a subpanel and have their own controls used to display and modify their values.
 */
public class EmitterPropertiesPanel extends Panel {
  public enum ShownProperty {
    DELAY("Delay"),
    LIFE_OFFSET("Life Offset"),
    X_OFFSET("X Offset"),
    Y_OFFSET("Y Offset"),
    VELOCITY("Velocity"),
    ANGLE("Angle"),
  ...
  public EmitterPropertiesPanel() {
    ...
    scrollPane.getStyle().hScrollKnob.setMinHeight(45);
    scrollPane.getStyle().hScrollKnob.setMinWidth(45);
    ...
    addTooltip(
        addPropertyTextButton,
        "Activate an optional emitter property",
        Align.left,
        Align.left,
        tooltipRightArrowStyle);
  }
  ...
  public void populateScrollTable(ShownProperty newProperty) {
    ...
    if (selectedEmitter.getDelay().isActive()) {
      ...
      var delaySubPanel =
          new RangeSubPanel(
              "Delay",
              selectedEmitter.getDelay(),
              "time from beginning of the effect to emission start in milliseconds",
              "change Delay",
              DELAY,
              20,
              500,
    ...
    // Duration
    scrollTable.row();
    var durationSubPanel =
        new RangeSubPanel(
            "Duration",
            selectedEmitter.getDuration(),
    ...
    // Emission
    scrollTable.row();
    var emissionSubPanel =
        new GraphSubPanel(
            "Emission",
            selectedEmitter.getEmission(),
            true,
            false,
    ...
    var lifeSubPanel =
        new GraphSubPanel(
            "Life",
            selectedEmitter.getLife(),
            true,
            true,
            "the time particles will live in milliseconds",
            "change Life",
            "Duration",
            null,
            20,
    ...
    if (selectedEmitter.getLifeOffset().isActive()) {
      scrollTable.row();
      var lifeOffsetSubPanel =
          new GraphSubPanel(
              "Life Offset",
              selectedEmitter.getLifeOffset(),
              true,
              true,
              "the life duration consumed upon particle emission in milliseconds",
              "change Life Offset",
              "Duration",
              LIFE_OFFSET,
              20,
    ...
    if (selectedEmitter.getYOffsetValue().isActive()) {
      ...
      var yOffsetSubPanel =
          new RangeSubPanel(
              "Y Offset",
              selectedEmitter.getYOffsetValue(),
    ...
    if (selectedEmitter.getVelocity().isActive()) {
      scrollTable.row();
      var velocitySubPanel =
          new GraphSubPanel(
              "Velocity",
              selectedEmitter.getVelocity(),
              true,
              false,
              "the particle speed in world units per second",
              "change Velocity",
              "Life",
              VELOCITY,
              5,
    ...
    if (selectedEmitter.getAngle().isActive()) {
      ...
      var angleSubPanel =
          new GraphSubPanel(
              "Angle",
              selectedEmitter.getAngle(),
              true,
              false,
              "the particle emission angle in degrees",
              "change Angle",
              "Life",
              ANGLE,
              5,
    ...
    if (selectedEmitter.getRotation().isActive()) {
      scrollTable.row();
      var rotationSubPanel =
          new GraphSubPanel(
              "Rotation",
              selectedEmitter.getRotation(),
              true,
              false,
              "the particle rotation in degrees",
              "change Rotation",
              "Life",
              ROTATION,
              15,
    ...
    if (selectedEmitter.getWind().isActive()) {
      ...
      var windSubPanel =
          new GraphSubPanel(
              "Wind",
              selectedEmitter.getWind(),
              true,
              false,
              "the wind strength in world units per second",
              "change Wind",
              "Life",
              WIND,
              5,
    ...
    if (selectedEmitter.getGravity().isActive()) {
      ...
      var gravitySubPanel =
          new GraphSubPanel(
              "Gravity",
              selectedEmitter.getGravity(),
              true,
              false,
              "the gravity strength in world units per second",
              "change Gravity",
              "Life",
              GRAVITY,
              5,
    ...
    if (scrollToActor != null) {
      ...
      scrollPane.scrollTo(
          0,
          temp.y + scrollToActor.getHeight(),
          scrollToActor.getWidth(),
      ...
      for (int i = 0; i < children.size; i++) {
        ...
        child.addAction(
            Actions.sequence(
                ...
                Actions.moveTo(child.getX(), child.getY(), .5f, Interpolation.smooth)));
      }
    }
  }
  ...
  public void removeProperty(ShownProperty shownProperty) {
    ...
    for (int i = 0; i < children.size; i++) {
      ...
      if (child.getUserObject() == shownProperty) {
        ...
        child.addAction(
            Actions.sequence(
                hideEmitterPropertyInTable(child), Actions.run(() -> populateScrollTable(null))));
        foundProperty = true;
